<script lang="ts">
    ...
    function getTeacher(id: number): string {
        ...
        return teacher ? `${teacher.name} ${teacher.father_lastname}` : "Maestro no encontrado";
    }

    $: groupSchedule = selectedGroupId ? Array.from(assignmentsMap.values()).filter(a => a.groupId === selectedGroupId) : [];
    ...
    function generatePDF() {
        ...
        const selectedGroup = groupsList.find(group => group.id === selectedGroupId);
        ...
        if (horarioElement && selectedGroup) {
            ...
            doc.text(`Grupo: ${selectedGroup.grade} ${selectedGroup.group}`, 10, 5);
